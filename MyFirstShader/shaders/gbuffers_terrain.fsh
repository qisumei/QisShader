...
void main() {
    ...
    if (matId == 50 || matId == 89 || matId == 169) emission = 5.0;
    if (matId == 42 || matId == 41 || matId == 57 || matId == 133 || matId == 152) metallic = 0.9;

    float sunVisibility = 0.0;
    if (skyLight > 0.05 && NdotL > 0.0 && emission == 1.0) {
    ...
    if (emission > 1.0) finalLight = vec3(1.0);

    outColor = vec4(color.rgb * finalLight * emission, color.a);

    outData = vec4(metallic, nView.x, nView.y, nView.z);
}